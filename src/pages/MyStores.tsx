...
const MyStores = () => {
  ...
  const { getMyStores, getStoreProducts, createStore, createProduct, updateStore, deleteStore, isLoading: storesLoading } = useStores();
  ...
  const handleFileSelect = (
    ...
    type: 'logo' | 'banner' | 'product' | 'edit-logo' | 'edit-banner'
  ) => {
                    ...
                    {getStoreProducts(store.id).length > 0 ? (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                        {getStoreProducts(store.id).map((product) => (
                          <div key={product.id} className="bg-muted/30 rounded-xl p-3 hover:bg-muted/50 transition-colors">
                            {product.image_url ? (
                              <img
